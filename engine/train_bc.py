...
def main(cfg: DictConfig):
    ...
    if not (cfg.dry or not fabric.is_global_zero):
        init_wandb(cfg)
    ...
    for epoch in metric_logger.log_every(range(cfg.epochs), 1, ""):
        ...
        if fabric.is_global_zero:
            ...
            if epoch % cfg.val_freq == 0:
                ...
                val_metrics = evaluate(model, val_loader, mix_precision=cfg.mix_precision, tag="val")
                ...
                if is_best:
                    print(f"New best model found at epoch {epoch} with loss {loss_metric:.4f}")
                    model.save(f"{work_dir}/model_best.ckpt")
                    with open(f"{work_dir}/best_epoch.txt", "w") as f:
                        f.write("Best epoch: %d, Best %s: %.4f" % (epoch, "loss", best_loss_logger.best_loss))
                if not cfg.dry:
        ...
        if epoch % cfg.save_freq == 0:
            ...
            if fabric.is_global_zero and hasattr(model, "forward_vis"):
                print(f"Visualizing and logging at epoch {epoch}...")
            ...
            if fabric.is_global_zero:
                ...
                if not cfg.dry:
                    wandb.log(gathered_results, step=epoch)
